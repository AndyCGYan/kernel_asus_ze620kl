...
#include <linux/proc_fs.h>		//ASUS_BSP PeterYeh : Add proc file node
...
static int mmc_can_poweroff_notify(const struct mmc_card *card);	//ASUS_BSP PeterYeh : eMMC porting
...
//ASUS_BSP PeterYeh : emmc info for ATD +++
static struct {
	u32 manfid;
	char *band_type;
} emmc_vendor_tbl[] = {
...
//ASUS_BSP PeterYeh : emmc info for ATD ---
//ASUS_BSP PeterYeh : Get eMMC health record in FOTA +++
...
static int mmc_decode_ext_csd(struct mmc_card *card, u8 *ext_csd)
{
	...
	//ASUS_BSP PeterYeh : add for fw version +++
...
//ASUS_BSP PeterYeh: turn off HPI +++
...
//ASUS_BSP PeterYeh: turn off BKOPS +++
...
//ASUS_BSP PeterYeh: turn off BKOPS ---
		 ...
		 */
//ASUS_BSP PeterYeh: turn off RPMB +++
...
//ASUS_BSP PeterYeh: turn off CACHE +++
...
//ASUS_BSP Peter: turn off PACKED CMD +++
	...
	if (card->ext_csd.rev >= 7) {
		...
		card->ext_csd.enhanced_rpmb_supported =
			(card->ext_csd.rel_param &
			 EXT_CSD_WR_REL_PARAM_EN_RPMB_REL_WR);
		//ASUS_BSP PeterYeh : add for life time of eMMC +++
		card->ext_csd.pre_eol_info = ext_csd[EXT_CSD_PRE_EOL_INFO];
...
//ASUS_BSP PeterYeh: "emmc info for ATD" ---
//ASUS_BSP PeterYeh : add for DEVICE_LIFE_TIME_EST_TYP of eMMC +++
...
//ASUS_BSP PeterYeh : dump eMMC health to log ---
	...
	if (mmc_card_support_auto_bkops(card)) {
		...
		//ASUS_BSP PeterYeh : dump eMMC info +++
...
//ASUS_BSP PeterYeh : Add proc file node +++
